...
class BatchDeleteSubmissionTask(SubmissionTask):
    ...
    def _submit(self, client, request_executor, transfer_future, **kwargs):
        ...
        # Process objects in batches of MAX_BATCH_SIZE (1000)
        for i in range(0, len(objects), MAX_BATCH_SIZE):
            batch = objects[i:i + MAX_BATCH_SIZE]
            
            # Submit a task for each batch
            self._transfer_coordinator.submit(
                request_executor,
                BatchDeleteObjectsTask(
                    ...
                    is_final=i + MAX_BATCH_SIZE >= len(objects),
                ),
            )
...
class BatchDeleteObjectsTask(Task):
    def _main(self, client, bucket, objects, extra_args):
        ...
        delete_request = {
            'Objects': objects,
            'Quiet': extra_args_copy.pop('Quiet', True),
        }
        
        # Make the delete-objects API call
        LOGGER.debug("I am in batch %s",delete_request)
        client.delete_objects(Bucket=bucket, Delete=delete_request, **extra_args_copy)
